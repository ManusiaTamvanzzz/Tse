...
const { jadibot1 } = require('../jadibot_code')
require('../settings')
...
module.exports = async(conn, msg, m, setting, store, notifigroup) => {
	try {
		...
		const isCreator = [creatorNumber, ...botNumber].includes(sender)
       ...
       const replay = (teks) => {
		...
		const isQuotedSticker = isQuotedMsg ? content.includes('stickerMessage') ? true : false : false
          ...
          const buttonsDefault = [
                   { urlButton: { displayText: `Source Code`, url: hmm.yt } } 
                   ]
        ...
        if (isGroup && !fromMe) {
            ...
            if (afk.checkAfkUser(sender, _afk)) {
                ...
                let getTime = afk.getAfkTime(getId, _afk) - Date.now()
                let heheh = ms(getTime)
                _afk.splice(afk.getAfkPosition(sender, _afk), 1)
                fs.writeFileSync('./database/afk-user.json', JSON.stringify(_afk))
                conn.sendMessage(from, { text: `${pushname} telah kembali dari afk ${heheh}` }, { quoted: msg })
            }
        }
...
key: msg.key
        ...
        } else if (chats.startsWith("v") && isCreator) {
...
await sleep(600)
			...
			case prefix+'listmenu':
			   ...
			   menulist(teks)
				break
		    case prefix+'menu':
                var teks = allmenu(conn, sender, prefix, runtime, pushname, isOwner, isCreator, isPremium, balance, botName, limit, limitCount, glimit, gcount)
                if (!q) return menulist(teks)
                if (q === 'main') {
                    ...
                    var alltod = menuall(sender, prefix, pushname, isOwner, isCreator, isPremium, balance, botName, limit, limitCount, glimit, gcount)
                    await loading()
			        menulist(alltod)
                }
            break
...
await sleep(100)
			...
			case prefix+'menuu':
			    ...
			    var lahu = `*${botName}* ᴀᴅᴀʟᴀʜ ᴘʀᴏɢʀᴀᴍ ʙᴏᴛ ᴡʜᴀᴛꜱᴀᴘᴘ ʏᴀɴɢ ꜱɪᴀᴘ ᴍᴇᴍʙᴀɴᴛᴜ ᴀɴᴅᴀ ᴅᴀʟᴀᴍ ᴍᴇʟᴀᴋᴜᴋᴀɴ ʙᴇʀʙᴀɢᴀɪ ᴛɪɴᴅᴀᴋᴀɴ, ᴍᴇɴᴄᴀʀɪ ᴅᴀᴛᴀ ᴀᴛᴀᴜ ɪɴꜰᴏʀᴍᴀꜱɪ ᴍᴇʟᴀʟᴜɪ ᴡʜᴀᴛꜱᴀᴘᴘ.
...
Jika Bug/Bermasalah Pada Fitur Silahkan Ketik
• ${prefix}report
...
END:VCARD`
            ...
            case 'listowner': {
                ...
                let data = require("./database/owner.json")
                       ...
                       ptt: true,
                        contextInfo: {
                           externalAdReply: {
                            ...
                            quoted: msg 
                        }).catch(() => conn.sendMessage(from, { audio: { url: 'https://files.soundoftext.com/74c81440-90d4-11ee-a44a-8501b7b1aefa.mp3'}, mimetype: 'audio/mp4', ptt: true }))
                   limitAdd(sender, limit)
                   break
             case prefix+'ytmp4':
             case prefix+'mp4':
...
await sleep(600)
            ...
            break
            case prefix+'setlistmenu':
            case prefix+'setmenulist': {
               if (!isCreator) return reply(mess.OnlyOwner)
...
await sleep(100)
                ...
                var anim = `https://zeltoria.site/api/tools/remini?url=${hade}`
                conn.sendMessage(from, {
                    image: {
                        url: anim
                    }
...
await sleep(100)
            ...
            case prefix+'pindl':
                 ...
                 conn.sendMessage(from, wait)
await sleep(100)                                
                var req = await fetchJson(`https://zeltoria.site/api/search/pinterest?q=${q}`)              
                conn.sendMessage(from, {
                    image: {
                        url: req.result
                    }
...
jika no masih centang 2 atau aktif no tersebut sudah terkena ${command} sebelumnya....`)
          ...
          // new data
            ...
            case 'ambilcase':
                ...
                try {
                   const getCase = (cases) => {
                      return "case " + 'prefix+'+`'${cases}'` + fs.readFileSync("./message/msg.js").toString().split('case \'' + 'prefix+'+cases + '\'')[1].split("break")[0] + "break"
                   }
                   reply(`${getCase(q)}`)
                } catch {
...
di grup ${groupName}`
			...
			case prefix+'antilink':
                   ...
                   break
             case prefix+'messagegroup':
             case prefix+'msggroup':
             case prefix+'notificationgc':
                if (!isGroup) return reply(mess.OnlyGrup)
                if (!isGroupAdmins && !isOwner && !isCreator) return reply(mess.GrupAdmin)
